	...
	, cancel(false)
{
	...
	::curl_easy_setopt(curl, CURLOPT_USERAGENT, "PresetManagement/0.0.1");
...
void Http::priv::http_perform(const HttpRetryOpt& retry_opts)
{
	...
	::curl_easy_setopt(curl, CURLOPT_VERBOSE, /*get_logging_level() >= 5*/ false);
	...
	do  {
        ...
        if (retry) {
            ...
            delay = (((delay * 2) < (retry_opts.max_delay)) ? (delay * 2) : (retry_opts.max_delay));
