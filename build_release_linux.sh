...
function check_available_memory_and_disk() {
    ...
    if [ ${FREE_MEM_GB} -le ${MIN_MEM_GB} ]; then
        echo -e "\nERROR: PrusaSlicer Builder requires at least ${MIN_MEM_GB}G of 'available' mem (system has only ${FREE_MEM_GB}G available)"
        echo && free -h && echo
        exit 2
    fi
...
echo "[1/9] Preparing build environment for dependencies..."
{
    # mkdir in deps
    if [ ! -d "deps/build" ]
    then
