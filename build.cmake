...
if(BUILD_TARGET STREQUAL "all" OR BUILD_TARGET STREQUAL "deps")
    ...
    if(CMAKE_HOST_SYSTEM_NAME STREQUAL "Darwin")
        ...
        list(APPEND DEPS_CMAKE_ARGS "-DCMAKE_TOOLCHAIN_FILE=${SOURCE_DIR}/cmake/toolchain-macos-disable-icu.cmake")
    endif()
...
if(BUILD_TARGET STREQUAL "all" OR BUILD_TARGET STREQUAL "slicer" OR BUILD_TARGET STREQUAL "install")
    ...
    # Set CMAKE_INSTALL_PREFIX to match the --install --prefix used below:
    # some install() destinations (e.g. resources) are written as
...
if(BUILD_TARGET STREQUAL "install")
    ...
    # Assemble the .app bundle on macOS. src/slic3r-app-launcher/CMakeLists.txt
    # (upstream) has no BUNDLE DESTINATION on its install(TARGETS ...) call,
    # and nothing sets MACOSX_BUNDLE_INFO_PLIST on the target -- 3.0 dropped
    # native CMake bundling for this target, unlike 2.9.x's PrusaSlicer
    # target. src/CMakeLists.txt still generates a working Info.plist
    # (CFBundleExecutable is "PrusaSlicer", from version.inc's
    # SLIC3R_APP_KEY), it's just never wired to a bundle. Assemble the same
    # Contents/MacOS + Contents/Resources structure here instead, matching
    # what boss's own 2.9.x bundle produced. The bundle must land inside
    # bin/, not at the top level of INSTALL_DIR: prusaslicer-ci's own DMG
    # packaging step (build_macos.yml) hardcodes
    # "${INSTALL_DIR}/bin/PrusaSlicer.app" and does
    # "hdiutil create -srcfolder ${INSTALL_DIR}/bin ..." with an
    # /Applications symlink placed alongside it in that same bin/ directory
    # -- matching 2.9.x's own install(TARGETS ... BUNDLE DESTINATION
    ...
    if(CMAKE_HOST_SYSTEM_NAME STREQUAL "Darwin")
        ...
        file(RENAME "${INSTALL_DIR}/resources" "${_bundle_contents}/Resources")
        file(COPY "${BUILD_DIR}/src/Info.plist" DESTINATION "${_bundle_contents}")

        # CFBundleIconFile must resolve directly under Contents/Resources; the
        # renamed resources tree only has it at Resources/icons/PrusaSlicer.icns.
        file(COPY "${_bundle_contents}/Resources/icons/PrusaSlicer.icns" DESTINATION "${_bundle_contents}/Resources")
    endif()
    ...
    if(CMAKE_HOST_SYSTEM_NAME STREQUAL "Darwin")
        ...
        # OCCTWrapper is built STATIC on macOS and linked into libslic3r;
        # the installed .a archive is unused at runtime — drop it from the bundle.
        file(GLOB_RECURSE _occt_a "${INSTALL_DIR}/OCCTWrapper.a")
        if(_occt_a)
            file(REMOVE ${_occt_a})
        endif()
    endif()
