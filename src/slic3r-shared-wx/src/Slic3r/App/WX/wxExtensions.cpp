...
wxBitmapBundle* get_bmp_bundle(const std::string& bmp_name_in, int width/* = 16*/, int height/* = -1*/, const std::string& new_color/* = std::string()*/)
{
    ...
    wxBitmapBundle* bmp = cache.from_svg(bmp_name, width, height, dark_mode(), new_color);
    if (bmp == nullptr) {
...
#else
    return cache.mksolid_bndl(width, height, color, 1, dark_mode());
#endif // __WXGTK2__
}
