...
{
    if (m_data.styles.empty()) return false;
    if (use_modification) {
...
void StyleManager::swap(size_t i1, size_t i2) {
    if (i1 >= m_data.styles.size() || 
        i2 >= m_data.styles.size()) return;
...
void StyleManager::erase(size_t index) {
    if (index >= m_data.styles.size()) return;
...
{
    if (style_index >= m_data.styles.size()) return false;
    if (!load_style(m_data.styles[style_index])) return false;
...
bool StyleManager::load_style(const Style &style) {
    if (style.emboss_style.descriptor.type == Domain::FontDescriptor::Type::file_path) {
        std::unique_ptr<Domain::FontFile> font_ptr =
            create_font_file(style.emboss_style.descriptor.path.c_str());
...
{
    if (m_style_cache.style_index >= m_data.styles.size()) return nullptr;
...
const StyleManager::Styles &StyleManager::get_styles() const{ return m_data.styles; }
...
void StyleManager::init_style_images(const Domain::Index2 &max_size,
                                    const std::string &text)
{
    ...
    for (const Style &style : m_data.styles) {        
        std::unique_ptr<const Domain::FontFile> font_file = m_font_manager.open(style.emboss_style.descriptor);
...
float StyleManager::get_imgui_font_size(const Domain::FontProp &prop, const Domain::FontFile &file, double scale)
{
...
void store_styles_obj(const std::string& path, StylesObj& data) {
    ...
    if (!file.is_open()) {
        BOOST_LOG_TRIVIAL(error) << "Text Preset can't be stored. "
            << "Failed to open cache file " << path << " for writing.";
        return;
    }
...
void make_unique_name(const Biz::Emboss::StyleManager::Styles& styles, std::string &name)
{
    auto is_unique = [&styles](const std::string &name){
        for (const Biz::Emboss::StyleManager::Style &it : styles)
            if (it.emboss_style.descriptor.name == name) return false;
        return true;
    };
...
Styles create_default_styles(Biz::Emboss::IFontManager& font_manager) {
    ...
    for (Domain::FontDescriptor& favorit : favorits) {
        ...
        styles.push_back(Biz::Emboss::StyleManager::Style{
            .emboss_style = Domain::EmbossStyle{.descriptor = favorit}
            });
    }
    return styles;
}
