    ...
    : m_session{dispatcher}
{
    ...
    if (!stored_data.access_token.empty()) {
        SPDLOG_INFO("{} access_token: {} ... {}", __FUNCTION__, stored_data.access_token.substr(0,5), stored_data.access_token.substr(stored_data.access_token.size()-5));
    } else {
...
void UserAccountCommunicationTokenBase::on_token_timer()
{
    ...
    if (expires_in_second >= 0) {
        ...
        m_slave_read_timer_id = Platform::PlatformServices::instance().timer_queue().set_timer(std::chrono::milliseconds((expires_in_second / 2) * 1000), std::bind(&UserAccountCommunicationTokenBase::on_slave_read_timer, this), false);
        return;
    }
...
{
    m_polling_timer_id = Platform::PlatformServices::instance().timer_queue().set_timer(std::chrono::milliseconds(10000), std::bind(&UserAccountCommunicationTokenBase::on_polling_timer, this), false);
...
void UserAccountCommunicationTokenBase::on_race_lost(const std::string& body)
{
    ...
    int miliseconds_to_timer = std::max((expires_in_second / 2) * 1000, (long long)2000); 
    m_after_race_lost_timer_id = Platform::PlatformServices::instance().timer_queue().set_timer(std::chrono::milliseconds(miliseconds_to_timer), std::bind(&UserAccountCommunicationTokenBase::on_after_race_lost_timer, this), false);
}
