...
public:
    explicit ProjectInteractor(Domain::Workbench& workbench)
        : m_workbench(workbench), m_preset_interactor(workbench)
    {
     ...
     */
    void select_project(SelectionId project_id);
     ...
     */
    SelectionId add_project(Domain::Project&& p);

    /**
     * Remove given project from workbench and update selection if needed.
     * @param project_id
     */
    void remove_project(SelectionId project_id);
...
private:
    void do_select_project(SelectionId project_id);
    void do_select_config_container(SelectionId container_id);
