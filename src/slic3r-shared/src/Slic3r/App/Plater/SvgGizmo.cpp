...
Yoga::GizmoWindowPtr SvgGizmo::release_ui_window() {
    ...
    callbacks.face_the_camera = [this]() {
        ...
        Domain::Transform3d new_volume_tr = Biz::Emboss::get_volume_transformation(to_world, wanted_dir, world_position, instance_tr_inv,
            proj_ctx.rotation, proj_ctx.from_surface, up_limit);
    ...
    auto set_svg_filepath = [this](const std::string& file_path) {
        ...
        if (res != Biz::Emboss::ReadShapeResult::success) {
            ...
            dialog_provider.show_error_dialog(Biz::Emboss::to_string(res, file_path), _u8L("Error loading SVG file"));
            return;
        }
        update_volume();
    };
    ...
    callbacks.forgot_filepath = [this]() {
        Domain::EmbossShape::SvgFile& svg_file = 
            *m_proj_ctxs->selected().shape.svg_file;
        svg_file.path.clear();
        dialog().set_filename(get_filename(svg_file));
...
void SvgGizmo::on_drag_cancel() { m_surface_drag.on_drag_cancel(); }
...
Domain::ModelVolume& get_selected_volume(Biz::ProjectInteractor& project_interactor) {
    ...
    const Domain::ElementRef& el = project_interactor.scene_interactor()
        .object_selection().elements.front();
    return (el.has_volume()) ?
...
const Domain::ModelVolume* get_selected_svg_volume(const Biz::ProjectInteractor& project_interactor) 
{
    ...
    return get_selected_svg_volume(project, selection);
}
...
void set_dialog_surface_distance(SvgDialog& dialog, const ProjectContext& proj_ctx) {
    ...
    if (const std::optional<float>& scale = proj_ctx.volume_scale.depth;
        scale.has_value())
        max_distance *= *scale;
    dialog.set_surface_distance(distance_in_mm, max_distance);
}

std::string parse_filename(const std::string& file_path) {
    if (file_path.empty())
        return file_path;
...
std::string get_filename(const Domain::EmbossShape::SvgFile& svg) {
    return (!svg.path.empty()) ?
        parse_filename(svg.path) :
        ((!svg.path_in_3mf.empty()) ?
            parse_filename(svg.path_in_3mf) :
            std::string(_u8L("--unknown--")));
}
...
Domain::Vec2d get_world_size(const ProjectContext& proj_ctx) {
    ...
    if (scale.width.has_value())  size.x() *= (*scale.width);
    if (scale.height.has_value()) size.y() *= (*scale.height);
    return size;
}
    ...
    const Domain::ModelVolume& volume)
{
    ...
    dialog.set_filename(get_filename(*shape.svg_file));
...
std::string create_fill_warning(const NSVGshape& shape) {
    ...
    if ((shape.opacity - 1.f + warning_preccission) <= 0.f)
        add_comma_separated(warning, fmt::format(fmt::runtime(
            _u8L("Opacity ({})")), shape.opacity));

    // if(shape->flags != NSVG_FLAGS_VISIBLE) add_warning(_u8L("Visibility flag"));
    bool is_fill_gradient = shape.fillGradient[0] != '\0';
    if (is_fill_gradient)
        add_comma_separated(warning, fmt::format(fmt::runtime(
            _u8L("Color gradient ({})")), shape.fillGradient));
...
std::string create_stroke_warning(const NSVGshape& shape, float min_scale) {
    ...
    if (float minimal_width_in_mm = 1e-3f;
        shape.strokeWidth <= minimal_width_in_mm * min_scale) {
        add_comma_separated(warning, fmt::format(fmt::runtime(
            _u8L("Too thin stroke (minimal width is {} mm).")), minimal_width_in_mm));
    }
    ...
    if (is_stroke_gradient)
        add_comma_separated(warning, fmt::format(fmt::runtime(
            _u8L("Color gradient ({})")), shape.strokeGradient));
...
std::string create_shape_warnings(const Domain::EmbossShape& shape, float min_scale) {
    ...
    for (NSVGshape* nsvg_shape = image.shapes; nsvg_shape != NULL; nsvg_shape = nsvg_shape->next, ++shape_index) {
        if (!(nsvg_shape->flags == NSVG_FLAGS_VISIBLE)) {
            add_warning(shape_index * 2, fmt::format(fmt::runtime(
                _u8L("Shape is marked as invisible ({}).")), nsvg_shape->id));
            continue;
        }        
        if (std::string fill_warning = create_fill_warning(*nsvg_shape);
            !fill_warning.empty()) {
            // TRN: The first placeholder is shape identifier, the second one is text describing the problem.
            add_warning(shape_index * 2, fmt::format(fmt::runtime(
                _u8L("Fill of shape ({}) contains unsupported: {}.")), nsvg_shape->id, fill_warning));
        }
        if (std::string stroke_warning = create_stroke_warning(*nsvg_shape, min_scale);
            !stroke_warning.empty())
            add_warning(shape_index * 2 + 1, fmt::format(fmt::runtime(
                _u8L("Stroke of shape ({}) contains unsupported: {}.")), nsvg_shape->id, stroke_warning));
    }
...
void SvgGizmo::on_scene_selection_changed(Domain::SelectionId project_id, const Biz::Scene::ObjectSelection& selection)
{
    ...
    const Domain::ModelVolume* volume_ptr = get_selected_svg_volume(project, selection);
    if (volume_ptr == nullptr)
    ...
    const Domain::ModelInstance* instance = Biz::Emboss::get_selected_instance(
        selection.elements, project);
...
void SvgGizmo::on_project_activated(size_t new_project_id)
{
    ...
    const Domain::ModelVolume* volume_ptr = get_selected_svg_volume(project, scene_interactor.object_selection());
    ASSERT(volume_ptr != nullptr); // cant activated without selected embossed svg volume
...
Biz::Emboss::BaseData::IssueFn create_issue_fn(
    ...
    return [prepend_tooltip](Biz::Emboss::JobIssue issue) {
        ...
        case JobIssue::no_shape:
            App::AppServices::instance().dialog_manager().show_error_dialog(_u8L("No shape, check SVG that contain path.")); break;
        case JobIssue::no_surface:     prepend_tooltip(_u8L("No surface, check correct position of the SVG.")); break;
        case JobIssue::canceled:       /* prepend_tooltip(_u8L("Job was canceled.")); */  break;
        default: prepend_tooltip(_u8L("Emboss job was not finished."));
        }
    };
}
