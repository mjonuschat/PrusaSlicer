...
bool CameraGizmo::pick_plane(double mouse_x, double mouse_y, const Render::ScreenInfo& screen_info, Vec3d& out_plane_point)
{
...
#if CAMERA_GIZMO_DEBUG
void CameraGizmo::render_scene(Render::CommandBuffer& cmd_buffer)
{
...
void CameraGizmo::look_at(const Vec3d& pos, double azimuth, double zenith)
{
    auto& scene = m_scene_provider.scene();
    auto& trackball = scene.camera_trackball();
