...
static LayoutButton* add_button(Item* parent, Render::Icon icon, const std::string& tooltip)
{
    ...
    // btn->set_padding(Paddings(0.f, 0.f, 10.f, 0.f));
    return btn;
}
...
Slic3r::App::Yoga::PartProcessingRow::~PartProcessingRow() {}
...
static LayoutButton* add_revert_btn(Item* parent, const std::string& tooltip)
{
    LayoutButton* revert_btn =
        parent->emplace_back<LayoutButton>("", Render::Icon::DSRevert, tooltip);
...
void CutDialog::init_cut_plane_input_panel()
{
    ...
    {
        m_is_planar_cut_mode = btn == m_planar_mode_btn;
...
void CutDialog::add_cut_settings()
{
    ...
    {
        bool keep_as_parts = btn == m_cut_into_parts_btn;
        m_part_A->set_as_part(keep_as_parts);
        m_part_B->set_as_part(keep_as_parts);
        if (callbacks().keep_as_part_changed) {
            callbacks().keep_as_part_changed(keep_as_parts);
        }
    };
...
void CutDialog::update_panels_visibility()
{
    ...
    m_groove_input_panel->set_visible(!m_is_planar_cut_mode);
}
...
void CutDialog::init_connectors_input_panel()
{
    ...
    m_dowel_btn =
        add_button(type_row, Render::Icon::DowelMarker, _u8L("Dowel"));
    ...
    Item* style_row = add_row(_u8L("Style"), m_connectors_input_panel);
    m_prism_btn   = add_button(style_row, Render::Icon::Prism, _u8L("Prism"));
    m_frustum_btn = add_button(style_row, Render::Icon::Frustum, _u8L("Frustum"));
    m_connector_style_group.set_buttons({m_prism_btn, m_frustum_btn});

    Item* shape_row = add_row(_u8L("Shape"), m_connectors_input_panel);
    m_triangle_btn = add_button(shape_row, Render::Icon::Triangle, _u8L("Triangle"));
    m_square_btn   = add_button(shape_row, Render::Icon::Square, _u8L("Square"));
    m_hexagon_btn  = add_button(shape_row, Render::Icon::Hexagon, _u8L("Hexagon"));
    m_circle_btn   = add_button(shape_row, Render::Icon::Circle, _u8L("Circle"));
    m_connector_shape_group.set_buttons({m_triangle_btn, m_square_btn, m_hexagon_btn, m_circle_btn});
    ...
    Item* rotation_row = add_row(_u8L("Groove Angle"), m_connectors_input_panel, std::string("°"));
    LayoutButton* revert_rotation_btn = add_revert_btn(rotation_row, _u8L("Revert groove angle"));
    ...
    for (SliderWithInput* input :
         {m_connector_depth_value, m_connector_size_value, m_connector_rotation})
    {
        input->set_width(120);
    }
...
void Plater::CutDialog::set_build_size(Domain::Vec3d size)
{
...
void Plater::CutDialog::set_current_connetor_type(Domain::CutConnectorType type)
{
...
void Plater::CutDialog::set_current_connetor_style(Domain::CutConnectorStyle style)
{
...
void Plater::CutDialog::set_current_connetor_shape(Domain::CutConnectorShape shape)
{
...
bool CutDialog::is_planar_mode() const
{
    return m_is_planar_cut_mode;
}
