...
void GCodeWindow::render_body(Yoga::Vec2f pos, Yoga::Vec2f size)
{
    ...
    if (!m_data->has_data()) {
        ...
        m_data->resize_range(visible_range, visible_lines_count, m_curr_line_id);
        ...
        if (ImGui::BeginTable("GCodeLines", 2, ImGuiTableFlags_SizingFixedFit)) {
            ...
            for (uint32_t id = *visible_range.min; id <= *visible_range.max; ++id) {
                ...
                ImGui::TableSetBgColor(ImGuiTableBgTarget_RowBg0,
                    ImGui::GetColorU32((id == m_curr_line_id) ? highlight_bg_color() : no_highlight_bg_color()));
...
void GCodeWindow::set_data(GCodeWindowData* data) { m_data = data; }

void GCodeWindow::set_curr_line_id(uint32_t curr_line_id) { m_curr_line_id = curr_line_id; }

void GCodeWindow::set_clip_text(bool clip_text) { m_clip_text = clip_text; }
} // namespace Slic3r::App::Preview
