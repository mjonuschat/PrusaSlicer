...
public:
    ...
    int foo() const { return m_state->foo; }

    bool updated() const { return m_updated; }

protected:
    void on_data_update() { m_updated = true; }
...
public:
    explicit DummyItemWithContext(size_t index, const DummyState& state, DummyFactoryContext* context)
        : Slic3r::Biz::DataObserver<DummyState>(index, state), m_context(context)
    {}
...
TEST_CASE_METHOD(ImGuiFixture, "ListView with context")
{
    ...
    ListView<
        DummyItemWithContext, DummyState,
