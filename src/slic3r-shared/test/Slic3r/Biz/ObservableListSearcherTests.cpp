...
static int substring_score(const std::string& item, const std::string& search_text)
{
...
TEST_CASE_METHOD(SearcherFixture, "[ObservableListSearcher] single-word search filters and orders by score")
{
    ...
    searcher->set_score_fn([](const std::string& item, const std::string& search_text) -> int {
        if (item == search_text) return 2;
        if (item.find(search_text) != std::string::npos) return 1;
        return 0;
    });
...
TEST_CASE_METHOD(SearcherFixture, "[ObservableListSearcher] multi-word search sums scores across words")
{
    ...
    searcher->set_score_fn([](const std::string& item, const std::string& word) -> int {
        int count = 0;
        size_t pos = 0;
