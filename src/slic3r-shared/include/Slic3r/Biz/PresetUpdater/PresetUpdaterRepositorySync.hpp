...
public:
	...
	void sync(const SharedRepositoryVector& repositories
...
private:
     ...
     */
	void stage_rencofigurations_from_resources(const boost::filesystem::path& source_dir
        , const AbstractPresetUpdaterRepository* repo
		, PresetUpdaterProcessStatus* process_status) const;

     void stage_not_installed_vendor_from_resources(const boost::filesystem::path& source_dir
        , const AbstractPresetUpdaterRepository* repo
		, PresetUpdaterProcessStatus* process_status
        , const PresetUpdaterIndex& source_index) const;

    void stage_installed_vendor_from_resources(const boost::filesystem::path& source_dir
        , const AbstractPresetUpdaterRepository* repo
		, PresetUpdaterProcessStatus* process_status
        , const PresetUpdaterIndex& source_index) const;

    /// Downloads index archive to temp_path. For each index calls sync_not_installed_vendor or sync_installed_vendor.
	void sync_repository(const boost::filesystem::path& temp_path
        , const AbstractPresetUpdaterRepository* repo
		, PresetUpdaterProcessStatus* process_status) const;
