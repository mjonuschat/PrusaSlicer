...
public:
    ...
    void set_sidebar_after_slice_render_fn(std::function<void(Vec2f, Vec2f)> render_fn) {
        m_cb_sidebar_after_slice_render = render_fn; }

    void set_layer_slider_render_fn(std::function<void(Vec2f, Vec2f)> render_fn) {
        m_cb_layer_slider_render = render_fn; }

    void set_gcode_slider_render_fn(std::function<void(Vec2f, Vec2f)> render_fn) {
...
private:
    ...
    std::function<void(Vec2f, Vec2f)> m_cb_sidebar_auto_reslice_render;
    std::function<void(Vec2f, Vec2f)> m_cb_sidebar_after_slice_render;
};
