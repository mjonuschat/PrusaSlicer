...
typedef std::function<void(Slic3r::Domain::CustomGCodeType)> CheckGCodeCallback;
...
typedef std::function<std::string(Slic3r::Domain::CustomGCodeType)> GetGCodeCallback;
...
struct TickCode
{
    ...
    int tick{ 0 };
    Slic3r::Domain::CustomGCodeType type{ Slic3r::Domain::CustomGCodeType::ColorChange };
    int extruder = 0;
    std::string color;
    std::string extra;
};
...
public:
    ...
    Slic3r::Domain::PrinterMode mode{ Slic3r::Domain::PrinterMode::Undefined };
    ...
    void set_ticks(const Slic3r::Domain::CustomGCodeInfo& custom_gcode_per_print_z);

    bool has_tick(int tick) const;
    bool add_tick(const int tick, Slic3r::Domain::CustomGCodeType type, int extruder, float print_z);
    bool edit_tick(std::set<TickCode>::iterator it, float print_z);
    void add_auto_color_change(Slic3r::Domain::PrinterMode main_mode, const int extruders_cnt, float print_z);
    void switch_code(Slic3r::Domain::CustomGCodeType type_from, Slic3r::Domain::CustomGCodeType type_to);
    bool switch_code_for_tick(std::set<TickCode>::iterator it, Slic3r::Domain::CustomGCodeType type_to, const int extruder);
    void erase_all_ticks_with_code(Slic3r::Domain::CustomGCodeType type);

    ConflictType is_conflict_tick(const TickCode& tick, Slic3r::Domain::PrinterMode main_mode, float print_z) const;

    int tick_from_value(float value, bool force_lower_bound = false) const;

    std::string gcode(Slic3r::Domain::CustomGCodeType type) const;

    // Get used extruders for tick.
    // Means all extruders(tools) which will be used during printing from current tick to the end
    std::set<int> used_extruders_for_tick(int tick, float print_z,
        Slic3r::Domain::PrinterMode force_mode = Slic3r::Domain::PrinterMode::Undefined) const;
    ...
    std::array<int, 2> active_extruders_for_tick(int tick, Slic3r::Domain::PrinterMode main_mode) const;
    ...
    // false -> otherwise
    bool check_ticks_changed_event(Slic3r::Domain::CustomGCodeType type, Slic3r::Domain::PrinterMode main_mode);

    // return true, if extruder sequence was changed
    bool edit_extruder_sequence(const int max_tick, Slic3r::Domain::PrinterMode main_mode);
...
private:
    bool has_tick_with_code(Slic3r::Domain::CustomGCodeType type);

    std::string color_for_tick(TickCode tick, Slic3r::Domain::CustomGCodeType type, const int extruder);
