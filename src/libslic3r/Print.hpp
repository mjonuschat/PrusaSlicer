...
public:
    ...
    // Convex hull of the outer edge of the outermost skirt loop. Empty if no skirt/brim was generated.
    // Useful for exclude-object style geometry declarations that must never be wired up as excludable.
    const Points&                     skirt_convex_hull() const { return m_skirt_convex_hull; }
    ...
    // Return wipe tower corners (and stabilization cone samples, if present) in world coordinates
    // (shifted and rotated), including the wipe tower brim.
