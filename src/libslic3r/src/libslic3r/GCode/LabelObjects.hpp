...
{
public:
    void init(const Print& print, Domain::LabelObjectsStyle label_object_style, Domain::GCodeFlavor gcode_flavor);
...
private:
    ...
    // Geometry-only entries for regions that must never be printed inside an EXCLUDE_OBJECT_START/
    // END block, so a host's "Cancel Object" can never remove them: e.g. the skirt/brim and wipe
    // tower. Only populated for the Klipper firmware flavor, the only supported protocol where
    // declaring an object's geometry is decoupled from marking it excludable.
