...
static ExtrusionEntityCollection traverse_loops_classic(const PerimeterGenerator::Parameters &params, const Polygons &lower_slices_polygons_cache, const PerimeterGeneratorLoops &loops, ThickPolylines &thin_walls)
{
    ...
    const bool brim_layer = (params.layer_id == 0 && params.object_config.brim_width.value > 0);
    ...
    const bool reverse_contour = reverse_layer || brim_layer;
    const bool reverse_hole = (reverse_layer && params.config.external_perimeters_first_holes) || brim_layer;
    ...
    ExPolygons                 &out_fill_expolygons)
{
    ...
    if (params.config.alternate_extra_perimeter && params.config.perimeters > 0 && params.layer_id % 2 == 1 && !params.spiral_vase && params.config.fill_density.value > 0)
        loop_number++;
