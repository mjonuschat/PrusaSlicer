    ...
    const std::function<void()>                              &throw_on_cancel_callback)
{
    ...
    for (const std::unique_ptr<PrintRegion>& pr : print_object_regions.all_regions) {
        if (pr.get()) {
            ...
            double scale = print_config.filament_shrink.get_at(extruder_id) / 100;
            if (scale != 1) {
                scale = 1 / scale;
...
void PrintObject::slice_volumes()
{
    ...
    std::vector<std::vector<ExPolygons>> region_slices = slices_to_regions(this->print()->config(), this->model_object()->volumes, *m_shared_regions, slice_zs,
        slice_volumes_inner(
