...
std::string WipeTowerIntegration::append_tcr(GCodeGenerator &gcodegen, const WipeTower::ToolChangeResult& tcr, int new_extruder_id, double z) const
{
    ...
    const bool is_ramming =
        (gcodegen.config().wipe_tower_disable_filament_ramming &&
    ...
    if (gcodegen.config().default_acceleration > 0) {
        gcode += gcodegen.writer().set_print_acceleration(
            ...
            gcodegen.config().wipe_tower_minimum_cruise_ratio.value,
            "Wipe Tower"
        );
    ...
    gcode += gcodegen.writer().set_print_acceleration(
        ...
        gcodegen.config().default_minimum_cruise_ratio.value,
        "Default"
    );
