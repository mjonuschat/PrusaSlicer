...
void GCodeProcessor::post_process()
{
    ...
    // Track the layer while rewriting the G-code stream so preheat lines use the layer
    // containing the T command being processed.
    unsigned int current_layer_id = 0;

    // add lines M104 to exported gcode
    auto process_line_T = [this, &export_lines, &current_layer_id](
        ...
        if (cmd.size() >= 2) {
            ...
            export_lines.insert_lines(backtrace, cmd,
                ...
                [tool_number, this, &current_layer_id](unsigned int id, const std::vector<float>& time_diffs) {
                    const int temperature = int(current_layer_id > 1 ?
                        m_extruder_temps_config[tool_number] :
        ...
        for (;;) {
            ...
            while (it != it_bufend || (eof && !gcode_line.empty())) {
                ...
                if (eol) {
                    ...
                    if (gcode_line == ";" + reserved_tag(ETags::Layer_Change) + "\n")
                        ++current_layer_id;
