...
//    const int                  infill_extruder      = region_config.infill_extruder.value;
    ...
    {
        double infill_width = region_config.infill_extrusion_width.get_abs_value(max_nozzle_diameter);
        if (infill_width < EPSILON)
            infill_width = default_infill_extrusion_width;
        m_infill_extrusion_width = scaled<float>(infill_width);
    }
