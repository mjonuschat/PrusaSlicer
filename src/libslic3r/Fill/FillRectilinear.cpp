...
//        if (sticks_removed) BOOST_LOG_TRIVIAL(error) << "Sticks removed!";
        ...
        for (size_t i = 0; i < n_contours; ++ i) {
            ...
            Slic3r::Biz::Algorithms::Polygon::remove_consecutive_duplicate_points(curr_contour, false);
            assert(!Slic3r::Biz::Algorithms::Polygon::has_consecutive_duplicate_points(curr_contour));
                                              ...
                                              Polylines                              &polylines_out)
{
    ...
    for (;;) {
        ...
        Slic3r::Biz::Algorithms::Polyline::remove_consecutive_duplicate_points(*polyline_current);
        assert(!Slic3r::Biz::Algorithms::Polyline::has_consecutive_duplicate_points(*polyline_current));
...
static void polylines_from_paths(const std::vector<MonotonicRegionLink> &path, const ExPolygonWithOffset &poly_with_offset, const std::vector<SegmentedIntersectionLine> &segs, Polylines &polylines_out)
{
	...
	auto finish_polyline = [&polyline, &polylines_out]() {
        Slic3r::Biz::Algorithms::Polyline::remove_consecutive_duplicate_points(*polyline);
        // Handle duplicate points and zero length segments.
        assert(!Slic3r::Biz::Algorithms::Polyline::has_consecutive_duplicate_points(*polyline));
        // Handle nearly zero length edges.
...
//    const float INFILL_OVERLAP_OVER_SPACING = 0.3f;
    ...
    for (Polylines::iterator it = polylines_out.begin() + n_polylines_out_initial; it != polylines_out.end(); ++ it) {
        ...
        assert(!Slic3r::Biz::Algorithms::Polyline::has_consecutive_duplicate_points(*it));
        it->rotate(rotate_vector.first);
        //FIXME rather simplify the paths to avoid very short edges?
        //assert(! it->has_duplicate_points());
        Slic3r::Biz::Algorithms::Polyline::remove_consecutive_duplicate_points(*it);
    }
