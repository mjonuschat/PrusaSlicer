...
void TopBarItemsCtrl::Button::set_selected(bool selected)
{
    ...
    m_foreground_color = m_is_selected ? w_config()->get_window_default_clr(): w_config()->get_label_clr_default() ;
    m_background_color = m_is_selected ? w_config()->get_label_clr_default() :
#ifdef _WIN32
                                         w_config()->get_window_default_clr();
#else
                                         wxTransparentColor;
#endif
...
void TopBarItemsCtrl::Button::set_hovered(bool hovered)
{
    ...
    m_background_color =    m_is_selected ? w_config()->get_label_clr_default()     :
                            hovered       ? w_config()->get_color_selected_btn_bg() :
