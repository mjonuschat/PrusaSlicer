...
void OptionsGroup::activate_line(Line& line)
{
    ...
    if (!custom_ctrl && m_use_custom_ctrl) {
        ...
        WX::w_config()->UpdateDarkUI(custom_ctrl);
		if (is_legend_line)
    ...
    if (label_width != 0) {
        if (custom_ctrl) {
            ...
            if (!line.near_label_widget || !line.label.IsEmpty()) {
                ...
                label->Wrap(label_width * WX::w_config()->em_unit()); // avoid a Linux/GTK bug
            }
            if (!line.near_label_widget)
        ...
        if (!custom_ctrl) {
            ...
            if (!option.sidetext.empty() || sidetext_width > 0) {
                auto sidetext = new wxStaticText(this->ctrl_parent(), wxID_ANY, _(option.sidetext), wxDefaultPosition,
                    wxSize(sidetext_width != -1 ? sidetext_width * WX::w_config()->em_unit() : -1, -1), wxALIGN_LEFT);
                sidetext->SetBackgroundStyle(wxBG_STYLE_PAINT);
                sidetext->SetFont(WX::w_config()->normal_font());
...
bool OptionsGroup::activate(std::function<void()> throw_if_canceled/* = [](){}*/, int horiz_alignment/* = wxALIGN_LEFT*/)
{
	...
	try {
		if (staticbox) {
			...
			stb->SetFont(wxOSX ? WX::w_config()->normal_font() : WX::w_config()->bold_font());
            WX::w_config()->UpdateDarkUI(stb);
		}
		else
			stb = nullptr;
    ...
    auto rescale = [](wxSizer* sizer) {
        for (wxSizerItem* item : sizer->GetChildren())
            if (item->IsWindow()) {
                ...
                WX::ScalableButton* sc_btn = dynamic_cast<WX::ScalableButton*>(win);
                if (sc_btn) {
...
#ifdef _WIN32
    if (staticbox && stb) {
        WX::w_config()->UpdateAllStaticTextDarkUI(stb);
        // update bitmaps for extra column items (like "delete" buttons on settings panel)
        for (auto extra_col : m_extra_column_item_ptrs)
            WX::w_config()->UpdateDarkUI(extra_col);
    }
    ...
    auto update = [](wxSizer* sizer) {
        for (wxSizerItem* item : sizer->GetChildren())
            if (item->IsWindow()) {
                wxWindow* win = item->GetWindow();
                // check if window is ScalableButton
                if (WX::ScalableButton* sc_btn = dynamic_cast<WX::ScalableButton*>(win)) {
                    sc_btn->sys_color_changed();
                    return;
                }
                WX::w_config()->UpdateDarkUI(win, dynamic_cast<wxButton*>(win) != nullptr);
            }
    };
    ...
    if (opt->nullable)
    {
        ...
        case coFloat:
            if (config.option(opt_key)->is_nil())
                ...
                ret = WX::double_to_string(config.option<ConfigOptionFloatNullable>(opt_key)->value);
...
{
    return true; //wxGetApp().open_browser_with_warning_dialog(OptionsGroup::get_url(path_end), wxGetApp().tab_panel());
}
...
{
	SetLabel(value);
    if (wrap) Wrap(60 * WX::w_config()->em_unit());
	GetParent()->Layout();
}
...
//        return;
    ...
    SetFont(focus ? WX::w_config()->link_font() :
