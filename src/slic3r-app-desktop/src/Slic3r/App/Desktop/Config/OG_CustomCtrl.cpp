    ...
    opt_group(og)
{
    ...
    m_bmp_mode_sz       = get_bitmap_size(WX::get_bmp_bundle("mode", wxOSX ? 10 : 12), this);
    m_bmp_blinking_sz   = get_bitmap_size(WX::get_bmp_bundle("search_blink"), this);
...
void OG_CustomCtrl::CtrlLine::msw_rescale()
{
    ...
    if (draw_just_act_buttons)
        height = get_bitmap_size(WX::get_bmp_bundle("empty"), ctrl).GetHeight();
...
void OG_CustomCtrl::CtrlLine::render(wxDC& dc, wxCoord v_pos)
{
    ...
    const bool suppress_hyperlinks = false;//!  get_app_config()->get_bool("suppress_hyperlinks");
    if (draw_just_act_buttons) {
        if (field) {
...
wxCoord OG_CustomCtrl::CtrlLine::draw_mode_bmp(wxDC& dc, wxCoord v_pos)
{
    ...
    wxBitmapBundle* bmp = WX::get_bmp_bundle("mode", pix_cnt, pix_cnt, WX::w_config()->get_mode_btn_color(mode));
...
#endif            
        dc.SetTextForeground(color ? *color : WX::w_config()->get_label_clr_default());
...
{
    wxBitmapBundle* bmp_blinking = WX::get_bmp_bundle(is_blinking ? "search_blink" : "empty");
    wxCoord h_pos = pos.x;
