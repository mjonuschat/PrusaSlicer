...
using text_ctrl = Slic3r::App::WX::Widgets::TextInput; //wxTextCtrl
...
class UndoValueUIManager
{
	struct UndoValueUI {
		...
		const WX::ScalableBitmap* undo_bitmap{ nullptr };
		const wxString* undo_tooltip{ nullptr };
		// Bitmap and Tooltip text for m_Undo_to_sys_btn. The wxButton will be updated only if the new wxBitmap pointer differs from the currently rendered one.
		const WX::ScalableBitmap* undo_to_sys_bitmap{ nullptr };
		...
		bool 	set_undo_bitmap(const WX::ScalableBitmap* bmp) {
			if (undo_bitmap != bmp) {
				undo_bitmap = bmp;
				return true;
			}
		...
		bool 	set_undo_to_sys_bitmap(const WX::ScalableBitmap* bmp) {
	...
	struct EditValueUI {
		...
		const WX::ScalableBitmap*	bitmap{ nullptr };
		wxString				tooltip { wxEmptyString };

		bool 	set_bitmap(const WX::ScalableBitmap* bmp) {
			if (bitmap != bmp) {
				bitmap = bmp;
				return true;
			}
...
public:
	...
	bool 	set_undo_bitmap(const WX::ScalableBitmap* bmp)			{ return m_undo_ui.set_undo_bitmap(bmp); }
	bool 	set_undo_to_sys_bitmap(const WX::ScalableBitmap* bmp)	{ return m_undo_ui.set_undo_to_sys_bitmap(bmp); }
	...
	bool 	set_edit_bitmap(const WX::ScalableBitmap* bmp)			{ return m_edit_ui.set_bitmap(bmp); }
