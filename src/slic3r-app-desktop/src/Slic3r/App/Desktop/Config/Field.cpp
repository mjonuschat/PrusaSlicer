        ...
        if ((m_opt.type == coFloatOrPercent || m_opt.type == coFloatsOrPercents) && !str.IsEmpty() &&  str.Last() != '%')
        {
            ...
            if (is_na_value) {
                ...
                set_value(WX::double_to_string(val), true);
                     ...
                     (m_value.empty() || WX::into_u8(str) != boost::any_cast<std::string>(m_value)))
            {
                if (!check_value) {
                    m_value.clear();
                    break;
...
void TextCtrl::BUILD() {
	...
	case coFloat:
	{
		...
		text_value = WX::double_to_string(val);
        m_last_meaningful_value = text_value;
		break;
	}
	case coString:
    ...
    temp->SetFont(m_opt.is_code ?
                  WX::w_config()->code_font():
                  WX::w_config()->normal_font());
    WX::w_config()->UpdateDarkUI(temp);
...
void TextCtrl::msw_rescale()
{
    ...
    if (size != wxDefaultSize) {
        if (text_ctrl* text_input = dynamic_cast<text_ctrl*>(window)) {
...
{
    if (WX::w_config()->suppress_round_corners())
        return new WX::Widgets::CheckBox(parent, label);
    
    return new WX::Widgets::SwitchButton(parent, label);
}

void CheckBox::SetValue(wxWindow* win, bool value)
{
    if (WX::w_config()->suppress_round_corners()) {
        if (WX::Widgets::CheckBox* ch_b = dynamic_cast<WX::Widgets::CheckBox*>(win))
            ch_b->SetValue(value);
    }
    else {
        if (WX::Widgets::SwitchButton* ch_b = dynamic_cast<WX::Widgets::SwitchButton*>(win))
            ch_b->SetValue(value);
    }
}
...
{
    if (WX::w_config()->suppress_round_corners())
        return dynamic_cast<WX::Widgets::CheckBox*>(win)->GetValue();

    return dynamic_cast<WX::Widgets::SwitchButton*>(win)->GetValue();
}

void CheckBox::Rescale(wxWindow* win)
{
    if (WX::w_config()->suppress_round_corners())
        dynamic_cast<WX::Widgets::CheckBox*>(win)->Rescale();
    else
        dynamic_cast<WX::Widgets::SwitchButton*>(win)->Rescale();
}

void CheckBox::SysColorChanged(wxWindow* win)
{
    if (!WX::w_config()->suppress_round_corners())
        dynamic_cast<WX::Widgets::SwitchButton*>(win)->SysColorChange();
}

void CheckBox::SetValue(bool value)
{
    if (WX::w_config()->suppress_round_corners())
        dynamic_cast<WX::Widgets::CheckBox*>(window)->SetValue(value);
    else
        dynamic_cast<WX::Widgets::SwitchButton*>(window)->SetValue(value);
}

bool CheckBox::GetValue()
{
    if (WX::w_config()->suppress_round_corners())
        return dynamic_cast<WX::Widgets::CheckBox*>(window)->GetValue();
...
void CheckBox::BUILD() {
    ...
    WX::w_config()->UpdateDarkUI(window);
	window->SetFont(WX::w_config()->normal_font());
	if (!wxOSX) 
...
void CheckBox::sys_color_changed()
{
    ...
    if (auto switch_btn = dynamic_cast<WX::Widgets::SwitchButton*>(window))
...
void SpinCtrl::set_value(const boost::any& value, bool change_event/* = false*/)
{
    ...
    if (m_opt.nullable) {
        ...
        if (m_is_na_val)
            dynamic_cast<WX::Widgets::SpinInput*>(window)->SetValue(na_value(true));
        else {
            m_last_meaningful_value = value;
            dynamic_cast<WX::Widgets::SpinInput*>(window)->SetValue(tmp_value);
        }
    }
    else
        dynamic_cast<WX::Widgets::SpinInput*>(window)->SetValue(tmp_value);
...
void SpinCtrl::set_last_meaningful_value()
{
    ...
    dynamic_cast<WX::Widgets::SpinInput*>(window)->SetValue(val);
    tmp_value = val;
    propagate_value();
}
...
{
    WX::Widgets::SpinInput* spin = static_cast<WX::Widgets::SpinInput*>(window);
...
void SpinCtrl::msw_rescale()
{
    ...
    auto field = dynamic_cast<WX::Widgets::SpinInput*>(window);
    if (parent_is_custom_ctrl)
...
void Choice::set_selection()
{
	...
	if (!text_value.IsEmpty()) {
		if (auto opt = m_opt.enum_def->value_to_index(WX::into_u8(text_value)); opt.has_value())
...
void Choice::set_value(const boost::any& value, bool change_event)
{
	...
	case coStrings: {
        ...
        int sel_idx = -1;
        if (m_opt.enum_def) {
            if (auto idx = m_opt.enum_def->label_to_index(WX::into_u8(text_value)); idx.has_value())
                sel_idx = *idx;
            else if (idx = m_opt.enum_def->value_to_index(WX::into_u8(text_value)); idx.has_value())
                sel_idx = *idx;
        }
...
boost::any& Choice::get_value()
{
	...
	if (m_opt.type == coEnum || m_opt.type == coEnums)
        ...
        if (ret_enum < 0 || ! m_opt.enum_def->has_values() || m_opt.type == coStrings ||
            (WX::into_u8(ret_str) != m_opt.enum_def->value(ret_enum) && ret_str != _(m_opt.enum_def->label(ret_enum))))
			// modifies ret_string!
            get_value_by_opt_type(ret_str);
...
void ColourPicker::BUILD()
{
    ...
    WX::w_config()->UpdateDarkUI(temp->GetPickerCtrl());
...
void ColourPicker::sys_color_changed()
{
	...
	if (wxWindow* win = this->getWindow())
		if (wxColourPickerCtrl* picker = dynamic_cast<wxColourPickerCtrl*>(win))
            WX::w_config()->UpdateDarkUI(picker->GetPickerCtrl(), true);
#endif
}
...
void PointCtrl::BUILD()
{
    ...
    x_textctrl->SetFont(WX::w_config()->normal_font());
	if (!wxOSX) x_textctrl->SetBackgroundStyle(wxBG_STYLE_PAINT);
	y_textctrl->SetFont(WX::w_config()->normal_font());
	...
	WX::w_config()->UpdateDarkUI(x_textctrl);
	WX::w_config()->UpdateDarkUI(y_textctrl);
	WX::w_config()->UpdateDarkUI(static_text_x, false, true);
	WX::w_config()->UpdateDarkUI(static_text_y, false, true);
...
void PointCtrl::sys_color_changed()
{
    ...
    for (wxSizerItem* item: sizer->GetChildren())
        if (item->IsWindow())
            WX::w_config()->UpdateDarkUI(item->GetWindow());
#endif
}
...
void StaticText::BUILD()
{
	...
	temp->SetFont(WX::w_config()->normal_font());
	temp->SetBackgroundStyle(wxBG_STYLE_PAINT);
    temp->SetFont(WX::w_config()->bold_font());
...
void SliderCtrl::BUILD()
{
	...
	m_slider->SetFont(WX::w_config()->normal_font());
