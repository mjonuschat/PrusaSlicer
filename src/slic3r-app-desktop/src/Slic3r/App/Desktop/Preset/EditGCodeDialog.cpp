...
//!        wxGetApp().open_browser_with_warning_dialog(event.GetLinkInfo().GetHref());
    ...
    const auto text_clr = WX::w_config()->get_label_clr_default();
    const auto bgr_clr_str = WX::w_config()->get_html_bg_color(parent);
    ...
    const wxString link = WX::format_wxstr("<a href = \"%1%\">%2%</a>", "help.prusa3d.com/article/macros_1775", _L("help page"));

    // TRN ConfigWizard : Downloader : %1% = "help page"
    const wxString main_text = WX::format_wxstr(_L("For more information about placeholders and its use visit our %1%."), link);
    ...
    html_window->SetPage(WX::format_wxstr(
    ...
    m_params_list->SetFont(WX::w_config()->code_font());
    WX::w_config()->UpdateDarkUI(m_params_list);
    ...
    m_gcode_editor->SetFont(WX::w_config()->code_font());
    m_gcode_editor->SetInsertionPointEnd();
    WX::w_config()->UpdateDarkUI(m_gcode_editor);
    ...
    m_param_label->SetFont(WX::w_config()->bold_font());

    m_param_description = new wxStaticText(this, wxID_ANY, wxEmptyString);

    wxStdDialogButtonSizer* btns = this->CreateStdDialogButtonSizer(wxOK | wxCANCEL);
    WX::w_config()->UpdateDarkUI(this->FindWindowById(wxID_OK, this));
    WX::w_config()->UpdateDarkUI(this->FindWindowById(wxID_CANCEL, this));
...
{
    return WX::into_u8(m_gcode_editor->GetValue());
}
...
void EditGCodeDialog::init_params_list(const std::string& custom_gcode_name)
{
    ...
    if (!m_cgp_other_slicing_states_config_def.empty()) {
        slicing_state = m_params_list->AppendGroup(_L("Slicing state"), "custom-gcode_slicing-state");
        for (const auto& [opt_key, def] : m_cgp_other_slicing_states_config_def.options)
    ...
    if (!specific_params.empty()) {
        ...
        wxDataViewItem group = m_params_list->AppendGroup(WX::format_wxstr(_L("Specific for %1%"), custom_gcode_name), "custom-gcode_gcode");
    ...
    if (!m_cgp_other_presets_config_def.empty())
        for (const auto& [opt_key, def] : m_cgp_other_presets_config_def.options)
    ...
    if (!opt_key.empty()) {
        ...
        if (const ConfigDef* config_def = full_config.def(); config_def && config_def->has(opt_key)) {
            ...
            for (const ConfigDef* config: std::initializer_list<const ConfigDef*> {
                    ...
                    &m_cgp_ro_slicing_states_config_def,
                    &m_cgp_rw_slicing_states_config_def,
                    &m_cgp_other_slicing_states_config_def,
                    &m_cgp_print_statistics_config_def,
                    &m_cgp_objects_info_config_def,
                    &m_cgp_dimensions_config_def,
                    &m_cgp_timestamps_config_def,
                    &m_cgp_other_presets_config_def
            }) {
...
void EditGCodeDialog::msw_rescale()
{
...
#ifdef __linux__
        variant << wxDataViewIconText(node->text, WX::get_bmp_bundle(node->icon_name)->GetIconFor(m_ctrl->GetParent()));
#else
...
ParamsViewCtrl::ParamsViewCtrl(wxWindow *parent, wxSize size)
    ...
    m_em_unit(WX::w_config()->em_unit(parent))
{
    WX::w_config()->UpdateDVCDarkUI(this);
