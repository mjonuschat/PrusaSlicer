...
void init_fn(ConfigDefinitions& defs)
{
    ...
    def->init_fn = [](ConfigItem& item) -> void { item.set_int(2); };

    def = defs.add("nozzle_high_flow", Bools);
    def->belongs_to = { "printer_settings"};
    def->init_fn = [](ConfigItem& item) { item.bools() = { true, false }; };
    ...
    def->init_fn = [](ConfigItem& item) { item.set_str("{input_filename_base}.gcode"); };
    ...
    def->init_fn = [](ConfigItem& item) { item.set_enum(MyEnum::One); };
}
