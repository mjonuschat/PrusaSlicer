...
static inline Orientation orient(
...
static inline bool is_ccw(const Domain::Polygon& poly)
{
...
inline bool ray_ray_intersection(
...
inline Domain::Vec2d foot_pt(const Domain::Line& iline, const Domain::Point& ipt)
{
...
inline double ray_point_distance_squared(const Domain::Line& iline, const Domain::Point& ipt)
{
    return (foot_pt(iline, ipt) - ipt.cast<double>()).squaredNorm();
}

inline double ray_point_distance(const Domain::Line& iline, const Domain::Point& ipt)
{
...
extern Eigen::Quaterniond rotation_xyz_diff(
...
// Is the angle close to a multiple of 90 degrees?
inline bool is_rotation_ninety_degrees(const Domain::Vec3d& rotation)
{
...
inline bool trafos_differ_in_rotation_by_z_and_mirroring_by_xy_only(
