...
Domain::TriangleMesh construct(indexed_triangle_set &&its, const Domain::RepairedMeshErrors& errors = Domain::RepairedMeshErrors{});
...
std::vector<Domain::Index3> its_face_edge_ids(const typename Domain::IndexedTriangleSetType<mesh_info>::type &its);

std::vector<Domain::Index3> its_face_edge_ids(const indexed_triangle_set &its, std::function<void()> throw_on_cancel_callback);

template<Domain::AdditionalMeshInfo mesh_info = Domain::AdditionalMeshInfo::None>
std::vector<Domain::Index3> its_face_edge_ids(const typename Domain::IndexedTriangleSetType<mesh_info>::type &its, const std::vector<char> &face_mask);

// Having the face neighbors available, assign unique edge IDs to face edges for chaining of polygons over slices.
std::vector<Domain::Index3> its_face_edge_ids(const indexed_triangle_set &its, std::vector<Domain::Index3> &face_neighbors, bool assign_unbound_edges = false, int *num_edges = nullptr);
...
std::vector<indexed_triangle_set> its_split(const indexed_triangle_set &its, std::vector<Domain::Index3> &face_neighbors);
...
size_t its_number_of_patches(const indexed_triangle_set &its, const std::vector<Domain::Index3> &face_neighbors);
...
void its_collect_mesh_projection_points_above(const indexed_triangle_set &its, const Domain::SquareMatrix3f &m, const float z, Domain::Points &all_pts);
void its_collect_mesh_projection_points_above(const indexed_triangle_set &its, const Domain::Transform3f &t, const float z, Domain::Points &all_pts);

// Calculate 2D convex hull of a transformed and clipped mesh. Uses the function above.
Domain::Polygon its_convex_hull_2d_above(const indexed_triangle_set &its, const Domain::SquareMatrix3f &m, const float z);
Domain::Polygon its_convex_hull_2d_above(const indexed_triangle_set &its, const Domain::Transform3f &t, const float z);
...
inline Domain::Vec3f face_normal(const stl_vertex vertex[3]) { return  (vertex[1] - vertex[0]).cross(vertex[2] - vertex[1]).normalized(); }
inline Domain::Vec3f face_normal_normalized(const stl_vertex vertex[3]) { return  face_normal(vertex).normalized(); }
...
namespace cereal {
    template <class Archive> struct specialize<Archive, Slic3r::Domain::TriangleMesh, cereal::specialization::non_member_load_save> {};
    template<class Archive> void load(Archive &archive, Slic3r::Domain::TriangleMesh &mesh) {
        archive.loadBinary(reinterpret_cast<char*>(const_cast<Slic3r::Domain::TriangleMeshStats*>(&mesh.stats())), sizeof(Slic3r::Domain::TriangleMeshStats));
        archive(mesh.its.indices, mesh.its.vertices);
    }
    template<class Archive> void save(Archive &archive, const Slic3r::Domain::TriangleMesh &mesh) {
        archive.saveBinary(reinterpret_cast<const char*>(&mesh.stats()), sizeof(Slic3r::Domain::TriangleMeshStats));
        archive(mesh.its.indices, mesh.its.vertices);
    }
}
