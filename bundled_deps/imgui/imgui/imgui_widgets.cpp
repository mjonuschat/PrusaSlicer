...
//
// {
bool ImGui::ColorEdit4(const char* label, float col[4], const char* current_label, const char* original_label, ImGuiColorEditFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (BeginPopup("picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                ImGuiColorEditFlags picker_flags = (flags_untouched & picker_flags_to_forward) | ImGuiColorEditFlags_DisplayMask_ | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_AlphaPreviewHalf;
                SetNextItemWidth(square_sz * 12.0f); // Use 256 + bar sizes?
                value_changed |= ColorPicker4("##picker", col, current_label, original_label, picker_flags, &g.ColorPickerRef.x);
            }
            EndPopup();
        }
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], const char* current_label, const char* original_label, ImGuiColorEditFlags flags, const float* ref_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        if (ref_col != NULL)
        {
            if (original_label != nullptr)
                Text("%s", original_label);
            else
                Text("Original");
            ImVec4 ref_col_v4(ref_col[0], ref_col[1], ref_col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : ref_col[3]);
            if (ColorButton("##original", ref_col_v4, (flags & sub_flags_to_forward), ImVec2(square_sz * 3, square_sz * 2)))
