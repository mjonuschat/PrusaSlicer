...
SCENARIO("Shells", "[Shells]") {
    GIVEN("20mm box") {
        auto test = [](const TestConfig &config){
            ...
            const double solid_infill_speed = config.tool.at(0).items.opt("solid_infill_speed").get<FloatOrPercentage>().float_value() * 60;
            ...
            const int bottom_solid_layers = config.tool.at(0).items.opt("bottom_solid_layers").get<int>();
            const int top_solid_layers    = config.tool.at(0).items.opt("top_solid_layers").get<int>();
        ...
        config.tool.at(0).items.opt("solid_infill_speed").set(FloatOrPercentage{99});
        config.tool.at(0).items.opt("top_solid_infill_speed").set(FloatOrPercentage{99});
        config.tool.at(0).items.opt("bridge_speed").set(72.0);
        ...
        WHEN("three top and bottom layers") {
            ...
            config.tool.at(0).items.opt("bottom_solid_layers").set(3);
            test(config);
        }
...
SCENARIO("Shells (from Perl)", "[Shells]") {
    GIVEN("V shape, Slic3r GH #1161") {
        ...
        config.filament.at(0).items.opt("cooling").set(false);
        config.tool.at(0).items.opt("bridge_speed").set(solid_speed);
        config.tool.at(0).items.opt("solid_infill_speed").set(FloatOrPercentage{solid_speed});
        config.tool.at(0).items.opt("top_solid_infill_speed").set(FloatOrPercentage{solid_speed});
        // to prevent speeds from being altered
        config.tool.at(0).items.opt("first_layer_speed").set(FloatOrPercentage{Percentage{100}});
