...
TEST_CASE("SmallAreaFlowCompensator passes dE through unmodified above the max length", "[boss][flow]")
{
    auto comp = make_default_compensator();
    CHECK(comp.modify_flow(100.0, 1.0, Slic3r::ExtrusionRole::SolidInfill) == 1.0);
}

TEST_CASE("SmallAreaFlowCompensator passes dE through unmodified for non-solid-infill roles", "[boss][flow]")
{
    auto comp = make_default_compensator();
    CHECK(comp.modify_flow(0.2, 1.0, Slic3r::ExtrusionRole::InternalInfill) == 1.0);
}

TEST_CASE("SmallAreaFlowCompensator scales dE for short SolidInfill/TopSolidInfill segments", "[boss][flow]")
{
    auto   comp   = make_default_compensator();
