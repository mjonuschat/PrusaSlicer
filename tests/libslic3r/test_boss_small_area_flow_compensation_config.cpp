...
const Domain::ConfigItemDef *find_option(const std::string &name)
{
    ...
    auto                             it   = std::find_if(
        defs.defs().begin(), defs.defs().end(), [&name](const Domain::ConfigItemDef &def) {
...
TEST_CASE("SmallAreaFlowCompensationFeature registers 21 options with correct bounds", "[boss][config]")
{
...
TEST_CASE("SmallAreaFlowCompensationFeature default lengths and factors match the 2.9.x curve", "[boss][config]")
{
    const double default_lengths[10] = {0, 0.2, 0.4, 0.6, 0.8, 1.5, 2, 3, 5, 10};
    const double default_factors[10] = {0, 0.4444, 0.6145, 0.7059, 0.7619, 0.8571, 0.8889, 0.9231, 0.9520, 1.0};
